...
class SelfAttention_left(nn.Module):
    def __init__(self,emb_size, num_attention_heads):
        super(SelfAttention_left,self).__init__()
        ...
        self.query = nn.Linear(emb_size, self.all_head_size)  # 384->384
    ...
    def forward(self, dp_emb, dp_mask, K, V, M):
        mixed_query_layer = self.query(dp_emb)  # (16,50,384)
        ...
        attention_scores = (attention_scores.transpose(-1, -2) - dp_mask).transpose(-1, -2)
        attention_scores = attention_scores - M  # (16,12,50,50)+(16,1,1,50) mask中补的部分是-10000，其它0，让补的部分很小
        ...
        return context_layer  #(16,50,384)


class Attention_left(nn.Module):
    def __init__(self, emb_size, num_attention_heads, hidden_size):
        super(Attention_left, self).__init__()
        self.selfAttention = SelfAttention_left(emb_size, num_attention_heads) #自注意力机制
    ...
    def forward(self,dp_emb, dp_mask, K, V, M):
        ...
        M = (1.0 - M) * 100000.0
        selfAttention_output = self.selfAttention(dp_emb, dp_mask, K, V, M) #(16,50,384)
        ...
        return layer_output  #(16，50，384)


class SelfAttention_right(nn.Module):
    def __init__(self,emb_size, num_attention_heads):
        super(SelfAttention_right, self).__init__()
        if emb_size % num_attention_heads != 0:
            raise ValueError(
                "The hidden size (%d) is not a multiple of the number of attention "
                "heads (%d)" % (emb_size, num_attention_heads))
...
class Attention_right(nn.Module):
    def __init__(self, emb_size, num_attention_heads, hidden_size):
        super(Attention_right, self).__init__()
        self.selfAttention = SelfAttention_right(emb_size, num_attention_heads) #自注意力机制
        self.attentonLayer = nn.Linear(emb_size, emb_size)
        self.dropout1 = nn.Dropout(0.1)
        self.LayerNorm = nn.LayerNorm(emb_size)
    ...
    def forward(self,dp_emb, K, V, M):
        M = M.unsqueeze(1).unsqueeze(2)
        M = (1.0 - M) * 100000.0
        selfAttention_output = self.selfAttention(dp_emb, K, V, M) #(16,50,384)
        attentionLayer_output = self.attentonLayer(selfAttention_output)  #(16,50,384)->(16,50,384) 线性层
        attentionLayer_dropout = self.dropout1(attentionLayer_output)
        attention_output = self.LayerNorm(attentionLayer_dropout + dp_emb)
