...
def train_model(model, optimizer, train_data):
    ...
    for i in tqdm(range(counts)):
        ...
        out = model(batch_data, device, 'affinity')
...
def run(task, model, optimizer, train_data, valid_data, test_data):
    ...
    test_max_PCC = 0
    res = []
    for epoch in range(1, config.epochs_affinity + 1):
        ...
        elif res2[2] == valid_max_PCC:
            count_decline = count_decline + 1
            if test_max_PCC < res3[2]:
                test_max_PCC = res3[2]
                res = res3
                torch.save(model.state_dict(), save_path)
        else:
        ...
        if epoch % config.decay_interval_affinity == 0:       # 每隔n次学习率衰减一次
            optimizer.param_groups[0]['lr'] *= config.lr_decay
    return res


if __name__ == "__main__":
    task = 'EC50'    # 'EC50', 'Kd', 'Ki', 'IC50'
    ...
    model.to(device)
    optimizer = optim.AdamW(model.parameters(), lr=config.lr_affinity, weight_decay=config.weight_decay)

    train_valid_data = load_data_affinity(task, 'train')
    test_data = load_data_affinity(task, 'test')
